...
class App extends React.Component {
  ...
  async fetchData() {
    try {
      let data = (await makeRequest(`users/${this.state.session.user}/events`,
      ...
      this.state.habits.map(async (item, index) => {
        const habitHistoryUrl = `events/${item._id}/history`
  ...
  checkHabit = async (value, index) => {
    let habits = this.state.habits
    const url = `events/${this.state.habits[index]._id}/history`
